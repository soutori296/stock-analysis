...
def run_backtest(df, market_cap):
    ...
    try:
        ...
        while i < n - 5: 
            ...
            low = row.get('Low') if 'Low' in row.index else row.get('low', np.nan)
            ...
            if sma5 > sma25 and low <= sma5: 
                ...
                for j in range(1, 11):
                    ...
                    future_high = future.get('High') if 'High' in future.index else future.get('high', np.nan)
                    future_low = future.get('Low') if 'Low' in future.index else future.get('low', np.nan) 
...
def get_stock_data(ticker):
    ...
    try:
        ...
        if abs_mdd <= 1.0:
            ...
            mdd_risk_deduct = -10
        elif abs_mdd > 4.0: # 4%超
            mdd_risk_deduct = -7
        elif abs_mdd > 2.0: # 2%超 (あなたのSL超え)
            mdd_risk_deduct = -5
        ...
        if is_market_alert:
            # MDD減点の強化
            if abs_mdd > 10.0 and mdd_risk_deduct < -15: mdd_risk_deduct = -15
            elif abs_mdd > 7.0 and mdd_risk_deduct < -15: mdd_risk_deduct = -15
        ...
        return {
            "code": ticker, "name": info["name"], "price": curr_price, "cap_val": info.get("cap", 0),
            "cap_disp": fmt_market_cap(info.get("cap", 0)), "per": info["per"], "pbr": info["pbr"],
...
if st.session_state.analyzed_data:
    ...
    def create_table(d_list, title):
        ...
        headers = [
            ...
            ("企業名", "130px", None), 
            ("時価総額", "85px", None), 
            ("点", "35px", "AIスコア。最大DD率が-1%以下で+5点ボーナス。以降、段階的に減点。市場過熱時は減点強化。"), 
