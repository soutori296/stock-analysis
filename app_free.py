        ...
        if is_breakout:
            ...
            if "順" in strategy: 
                ...
                if not is_aoteng:
                    ...
                    # 青天井(青天)ブレイク：
                    # AIコメントと整合性を取るため、ATR_SL(1.5倍)を損切りラインとして強制適用する
                    # (元々のp_fullには2.5倍の深いSLが入っているため上書きする)
                    if atr_sl_price > 0:
                        sl_ma = int(atr_sl_price)
                        # p_fullもこれに合わせておくとターゲット欄のSL表示も揃う
                        p_full = sl_ma 
                    p_half = 0 
                    
                # SLが変更されたので、乖離率(sl_pct)も再計算する
