...
with col_title:
    ...
    <style>
        ...
        /* 4.時価総額 */
        th:nth-child(4), td:nth-child(4) { width: 75px; text-align: right; font-size: 12px; }

        /* 5.スコア */
        th:nth-child(5), td:nth-child(5) { width: 40px; text-align: center; }

        /* 6.戦略 */
        th:nth-child(6), td:nth-child(6) { width: 70px; font-size: 12px; }

        /* 7.RSI */
        th:nth-child(7), td:nth-child(7) { width: 60px; text-align: center; }

        /* 8.出来高 */
        th:nth-child(8), td:nth-child(8) { width: 70px; font-size: 12px; text-align: right; }

        /* 9.現在値 */
        th:nth-child(9), td:nth-child(9) { width: 75px; text-align: right; font-weight: bold; }

        /* 10.推奨買値 */
        th:nth-child(10), td:nth-child(10) { width: 90px; font-size: 12px; }

        /* 11.利確 */
        th:nth-child(11), td:nth-child(11) { width: 100px; font-size: 11px; }
        ...
        /* 13.PER/PBR (2段) */
        th:nth-child(13), td:nth-child(13) { width: 65px; font-size: 11px; line-height: 1.2 !important; }
...
# ヘルプ
with st.expander("ℹ️ バックテスト(押し目買い検証)のロジック詳細"):
    st.markdown("""
...
def generate_ranking_table(high_score_list, low_score_list):
    ...
    【重要：表のフォーマット】
    - 必ずヘッダーとデータ行の間に `<br>` を入れて2段組みにする箇所があります。
    ...
    **【買い推奨・注目ゾーン】**
    ...
    ※アイの所感は、所感以外の列を圧迫しないよう、少し短め（80文字程度）にしてください。
