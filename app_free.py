...
with col_title:
    ...
    <style>
        ...
        th:nth-child(4), td:nth-child(4) {{ width: 65px; text-align: right; }} /* 時価総額 */
...
def get_technical_summary(ticker):
    ...
    try:
        ...
        buy_display = f"{buy_target_val:,.0f} ({diff_txt})" # 【修正】<br>排除
...
def generate_ranking_table(high_score_list, low_score_list):
    ...
    【2段表示の指示】
    以下の項目は、AI側で `<br>` タグを挿入して2段に整形してください。
...
if st.session_state.analyzed_data:
    ...
    with st.spinner("🤖 アイが分析レポートを作成中... (並べ替え反映)"):
        ...
        if "<!--SEP-->" in main_part:
            ...
            table_1_rows = main_part # 全部上に入れる

        # テーブルHTML生成関数 (再利用)
        def create_html_table(rows):
            if not rows.strip(): return ""
            rows = rows.replace("```html", "").replace("```", "").strip()
            return f"""
            <table style="width: 100%; border-collapse: collapse; table-layout: fixed; font-family: 'Meiryo', sans-serif;">
              <thead>
                <tr style="background-color: #dcdcdc; color: #000000; border: 1px solid #bbbbbb;">
                  ...
                  <th style="width: 150px; text-align:left; padding:4px 1px; border:1px solid #bbb; font-size:11px;">企業名</th>
                  <th style="width: 90px; text-align:center; padding:4px 1px; border:1px solid #bbb; font-size:11px;">時価総額</th>
                  ...
                  <th style="width: 55px; text-align:center; padding:4px 1px; border:1px solid #bbb; font-size:11px;">出来高<br>(前日比)</th>
                  <th style="width: 65px; text-align:center; padding:4px 1px; border:1px solid #bbb; font-size:11px;">現在値</th>
                  <th style="width: 85px; text-align:center; padding:4px 1px; border:1px solid #bbb; font-size:11px;">推奨買値<br>(残)</th>
                  <th style="width: 125px; text-align:center; padding:4px 1px; border:1px solid #bbb; font-size:11px;">利確<br>(半/全)</th>
    ...
    if table_1_rows.strip():
        st.markdown(create_html_table(table_1_rows), unsafe_allow_html=True)
    else:
        st.info("推奨ゾーンの銘柄はありません。")

    st.markdown("#### ⚠️ 様子見・警戒ゾーン")
    if table_2_rows.strip():
        st.markdown(create_html_table(table_2_rows), unsafe_allow_html=True)
    else:
        st.info("警戒ゾーンの銘柄はありません。")
