...
if st.session_state.analyzed_data:
    ...
    def create_table(d_list, title):
        ...
        headers = [
            ...
            ("企業名", "125px", None), 
            ("時価総額", "95px", None), 
            ("点", "35px", "総合分析点。モデルが判断するリスク・リターン整合性のスコア。**売買推奨を示すものではありません。**"), # ★ 修正
            ("分析戦略", "75px", "🔥順張り: 上昇トレンド（MA）時の押し目待ちモデル。🌊逆張り: RSI低位や長期MA乖離時の反発待ちモデル。"), # ★ 修正
            ("現在値", "60px", None), 
            ("想定水準\n(乖離)", "65px", "この分析モデルが買付を「想定」するテクニカル水準。乖離は現在値との差額。**売買判断はご自身の責任で行ってください。**"), # ★ 修正
            ...
            ("過去実績\n(勝敗)", "70px", "過去75日間で、**「想定水準」での買付が「目標値」に到達した実績**。将来の勝敗を保証するものではありません。"), # ★ 修正
