...
with st.expander("ℹ️ スコア配分・利確計算ロジックの説明書を見る"):
    ...
    **基本点: 50点** からスタートし、以下の3要素で加点・減点を行います。
    ...
    ### 💰 利確ターゲット計算ロジック
    **【重要】 基準価格について**
    利確目標は、推奨買値（指値）ではなく **「現在値」を基準** に算出しています。
    *   理由: 指値を基準に計算すると、指値が現在値より低い場合に「目標株価が現在値より下（＝到達済み）」と表示され、上値余地が見えなくなるためです。
    *   意味: **「今の株価から見て、最低でもあとこれくらいの上値余地を狙う」** という目安です。
    ...
    *   **半益**: **5日移動平均線** (短期的な戻り)
    *   **全益**: **25日移動平均線** (平均値への回帰)
    ※もし現在値がすでに5日/25日線を超えている場合は、「現在値+3%」等を仮ターゲットとして設定します。
    """)
