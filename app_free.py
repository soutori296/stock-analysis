...
with st.expander("ℹ️ スコア配分・利確計算ロジックの説明書を見る"):
    ...
    **基本点: 50点** からスタートし、以下の3要素で加点・減点を行います。
    ...
    ---
    ### 💰 利確ターゲット計算ロジック
    戦略に応じて、自動的に以下の計算式でターゲットを設定しています。

    #### 🅰️ 🔥順張り (Trend Follow) の場合
    トレンドを伸ばすことを目的とします。
    *   **半益 (50%決済)**: 「現在値+5%」または「25日線+10%乖離」のうち、高い方を採用。
    *   **全益 (全決済)**: 「現在値+10%」または「25日線+20%乖離」のうち、高い方を採用。

    #### 🅱️ 🌊逆張り (Rebound) の場合
    自律反発（リバウンド）を狙うため、目標は控えめです。
    *   **半益**: **5日移動平均線** (短期的な戻り)
    *   **全益**: **25日移動平均線** (平均値への回帰)
    ※もし現在値がすでに5日線を超えている場合は、直近価格+3%程度を仮設定します。
    """)
...
def get_technical_summary(ticker):
    ...
    try:
        ...
        # 【修正】f文字列の書式指定だけで符号をつける
        # diff_txt = f"{diff:+,.0f}" これで +100 や -50 が自動で入る
        # 前回のコードで冗長な sign 変数があったため削除
        diff_txt = f"{diff:+,.0f}" if diff != 0 else "0"
        
        if strategy == "👀様子見":
            buy_price_display = "様子見推奨"
        else:
            # ここを修正: 不要な sign 変数を削除し、diff_txt のみを使用
