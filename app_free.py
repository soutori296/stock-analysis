...
def batch_analyze_with_ai(data_list):
    ...
    for d in data_list:
        ...
        prompt_text += f"ID:{d['code']} | {d['name']} | 現在:{price:,.0f} | 分析戦略:{d['strategy']} | RSI:{d['rsi']:.1f} | 5MA乖離率:{ma_div:+.1f}% | {rr_disp} | 出来高倍率:{d['vol_ratio']:.1f}倍 | リスク情報: MDD:{mdd:+.1f}%, MA75乖離率:{sl_pct:+.1f}% | {sl_ma_disp} | {low_liquidity_status} | 総合分析点:{d['score']}\n" 
    ...
    3.  <b>総合分析点に応じた文章量とトーンを厳格に調整してください。</b>
    ...
    6.  **【最重要: リスク情報と撤退基準・強調表現の制限】**
        ...
        - **強調表現の制限**: **総合分析点85点以上**の銘柄コメントに限り、**全体の5%の割合**（例: 20銘柄中1つ程度）で、特に重要な部分（例：出来高増加の事実、高い整合性）を**1箇所（10文字以内）**に限り、**赤太字のHTMLタグ（<b><span style="color:red;">...</span></b>）**を使用して強調しても良い。それ以外のコメントでは赤太字を絶対に使用しないでください。85点未満は<b>黒太字</b>のみ使用してください。
    ...
    【最後に】
    リストの最後に「END_OF_LIST」と書き、その後に続けて「アイの独り言（常体・独白調）」を3行程度で書いてください。語尾に「ね」や「だわ」などは使用しないこと。
    ※見出し不要。
    独り言の内容：
...
if st.session_state.analyzed_data:
    ...
    def create_table(d_list, title):
        ...
        headers = [
            ...
            ("企業名", "125px", None), 
            ("時価総額", "95px", None), 
            ("点", "35px", "総合分析点。モデルが判断するリスク・リターン整合性のスコア。<b>売買推奨を示すものではありません。</b>*"), # ★ 修正
            ("分析戦略", "75px", "🔥順張り: 上昇トレンド（MA）時の押し目待ちモデル。🌊逆張り: RSI低位や長期MA乖離時の反発待ちモデル。"), # ★ 修正
            ("現在値", "60px", None), 
            ("想定水準\n(乖離)", "65px", "この分析モデルが買付を「想定」するテクニカル水準。乖離は現在値との差額。<b>売買判断はご自身の責任で行ってください。</b>"), # ★ 修正
            ("R/R比", "40px", "想定水準から利益確定目標までの値幅を、SL MAまでの値幅で割った比率。1.0未満は-25点。"), # ★ 修正
            ("最大DD率\nSL乖離率", "70px", "最大DD率: 過去の同条件トレードでの最大下落率。SL乖離率: SLライン（過去の支持線）までの余地。"), 
            ("利益確定\n目標値", "120px", "時価総額別の分析リターンに基づき、利益確定の「目標値」として算出した水準。達成目安としてご参照ください。"), # ★ 修正
            ("RSI", "50px", "相対力指数。🔵30以下(売られすぎ) / 🟢55-65(上昇トレンド) / 🔴70以上(過熱)"), 
            ("出来高比\n（5日平均）", "80px", "上段は当日の出来高と5日平均出来高（補正済み）の比率。下段は5日平均出来高。1000株未満は-30点。"), 
            ("過去実績\n(勝敗)", "70px", "過去75日間で、「想定水準」での買付が「目標値」に到達した実績。<b>将来の勝敗を保証するものではありません。</b>"), # ★ 修正
